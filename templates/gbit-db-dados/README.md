...
# gbit-db-dados conecta automaticamente com o backend gbit-database
...
Motor de banco de dados local — publicado no npm, e já **integrado neste projeto** pelo CLI `gbit-db` pra você ver na prática como ele funciona.

Zero dependências, zero servidor externo. Só arquivos, rodando com Node.js.

## Já está pronto aqui

Essa pasta já é um banco de dados funcionando. Você não precisa configurar nada — é só criar suas coleções e usar:

```bash
npx gbit-db-dados create-collection users
npx gbit-db-dados list-collections
npx gbit-db-dados insert users '{"email":"a@a.com","name":"Ana"}'
npx gbit-db-dados find users
npx gbit-db-dados stats
```
...
# biblioteca para projeto externo  gbit-db-dados:

Ou direto no código do seu backend:
...
users.insert({ email: 'a@a.com', name: 'Ana' });
users.find({});
```

## Quer usar em outro projeto?

O `gbit-db-dados` não é exclusivo desse projeto gerado pelo `gbit-db` — ele é um pacote publicado no npm e funciona em **qualquer backend Node.js**, sem precisar instalar nada antes. Só rodar:

```bash
npx gbit-db-dados
```

Isso já baixa e executa o `gbit-db-dados` na hora, sem precisar dar `npm install` primeiro — e mostra todos os comandos disponíveis.

Pra já criar um banco de verdade dentro do seu projeto:

```bash
npx gbit-db-dados init ./meu-banco
```

Isso já cria a estrutura toda (`database.json`, `collections/`, `indexes/`, `transactions/`, `backups/`) e deixa o banco pronto pra uso — no terminal ou como biblioteca:
...
├── database.json        # metadados do banco
├── metadata.json         # schema e índices de cada coleção
├── collections/            # documentos de cada coleção
├── indexes/                  # índices persistidos
├── transactions/               # log de commits e rollbacks
└── backups/                       # snapshots de segurança
```

## Documentação completa

CLI, API da biblioteca, operadores de consulta, schema, validação e transações:
[gbit-db-dados no GitHub](https://github.com/Gislaine-programadora/gbit-db-dados) · [gbit-db-dados no npm](https://www.npmjs.com/package/gbit-db-dados)
